...
namespace DAL
{
    public static class DalEmprunt
    {
        ...
        /// <param name="listToReturn"></param>
        public static void GetRetardsByLib(int LibId, ref List<Emprunt> listToReturn)
        {
            DataTable dataTemp = new DataTable();
            ...
            using (SqlConnection connection = UtilsDAL.GetConnection())
            {
                ...
                try
                {
                    using (SqlCommand command = new SqlCommand("SchAdmin.CloseEmprunt", connection))
                    {
                        ...
                        if (rowsAffected == 0) throw new EL.CstmError(13); //  La modification n'a pas pu être effectuée ! \n Veuillez réessayer.
